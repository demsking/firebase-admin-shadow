...
class Reference extends Query {
  ...
  child (path) {
    ...
    $.references[key] = Reference.get(key, this.app)
  ...
  update (data, onComplete = null) {
    ...
    if ($.listeners.hasOwnProperty(this.key)) {
      $.listeners[this.key].emit('child_changed', this.snapshot)
    }
  ...
  on (eventType, callback) {
    if (!supportedEventNames.includes(eventType)) {
      console.warn(`Not Yet Implemented: event '${eventType}'. Ignored`)
      return this
    }
  ...
  once (eventType, callback) {
    ...
    $.listeners[this.key].once(eventType, callback)
  ...
  off (eventType, callback) {
    if (!supportedEventNames.includes(eventType)) {
      return console.warn(`Not Yet Implemented: event '${eventType}'. Ignored`)
    }
